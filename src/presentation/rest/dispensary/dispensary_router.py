...
from src.domain.dispensary.schema import AllDispensaries, DispensaryResponseForPost, DispensaryResponseForPut
from src.domain.dispensary.schema import DispensaryResponseForGet
...
async def get_dispensaries(dispensary_id: int) -> DispensaryResponseForGet:
...
async def update_dispensary(
	...
	return await dispensary_service.update_dispensary_service(dispensary_id, dispensary_name, address)


@dispensary_router.patch("/{dispensary_id}", response_model=DispensaryResponseForPut, status_code=status.HTTP_200_OK)
async def update_dispensary_status(dispensary_id: int) -> DispensaryResponseForPut:
	return await dispensary_service.update_dispensary_status_service(dispensary_id)
